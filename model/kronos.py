...
# ============================================================
# 修复 calc_time_stamps：兼容 DatetimeIndex 和 Series
# ============================================================
def calc_time_stamps(timestamps):
    """
    安全计算时间特征，兼容 Series、DatetimeIndex、ndarray、list
    """
    # 统一转为 pandas Series (dtype=datetime64)
    if isinstance(timestamps, pd.DatetimeIndex):
        ts = timestamps.to_series()
    elif isinstance(timestamps, pd.Series):
        ts = timestamps.reset_index(drop=True)
    else:
        # ndarray / list / 其他
        ts = pd.Series(pd.to_datetime(np.asarray(timestamps).ravel()))

    time_df = pd.DataFrame(index=ts.index)
    time_df['minute']   = ts.dt.minute.values
    time_df['hour']     = ts.dt.hour.values
    time_df['weekday']  = ts.dt.weekday.values
    time_df['day']      = ts.dt.day.values
    time_df['month']    = ts.dt.month.values
    return time_df
